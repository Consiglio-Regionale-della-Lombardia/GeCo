BEGIN
	...
	IF (NOT EXISTS (SELECT * 
                 ...
                 WHERE TABLE_NAME = 'join_persona_chisura'))
		BEGIN
	...
	IF (NOT EXISTS (SELECT * 
                 FROM INFORMATION_SCHEMA.TABLES 
                 WHERE TABLE_NAME = 'join_legislature_chiusura'))
		BEGIN
	...
	IF (NOT EXISTS (SELECT * 
                 FROM INFORMATION_SCHEMA.TABLES 
                 WHERE TABLE_NAME = 'join_persona_tessere'))
		BEGIN
				CREATE TABLE [dbo].[join_persona_tessere](
			[id_rec] [int] IDENTITY(1,1) NOT NULL,
	...
	IF NOT EXISTS(
		  ...
		  FROM INFORMATION_SCHEMA.COLUMNS
		  WHERE 
			[TABLE_NAME] = 'join_persona_organo_carica'
			AND [COLUMN_NAME] = 'chiuso')
			BEGIN
	...
	IF NOT EXISTS(
		  ...
		  FROM INFORMATION_SCHEMA.COLUMNS
		  WHERE 
			[TABLE_NAME] = 'join_persona_gruppi_politici'
			AND [COLUMN_NAME] = 'chiuso')
			BEGIN
	...
	IF NOT EXISTS(
		  ...
		  FROM INFORMATION_SCHEMA.COLUMNS
		  WHERE 
			[TABLE_NAME] = 'join_persona_organo_carica_priorita'
			AND [COLUMN_NAME] = 'chiuso')
			BEGIN
	...
	IF NOT EXISTS(
			...
			FROM INFORMATION_SCHEMA.COLUMNS
			WHERE 
			[TABLE_NAME] = 'join_gruppi_politici_legislature'
			AND [COLUMN_NAME] = 'chiuso')
			BEGIN
	...
	IF NOT EXISTS(
			...
			END
	COMMIT TRANSACTION
	print('PARTE 1 ESEGUITA CORRETTAMENTE')
	END TRY
	BEGIN CATCH
        ROLLBACK TRANSACTION
		print('ROLLBACK PARTE 1')
	END CATCH
END
GO
BEGIN
	BEGIN TRY 
	BEGIN TRANSACTION
	EXEC('CREATE PROCEDURE [dbo].[spChiusuraPersona] (@idPersona int, @idLegislatura int, @idCausaFine int, @dataChiusura datetime)
	AS
	BEGIN
	...
	EXEC('CREATE PROCEDURE [dbo].[spChiusuraLegislatura] @idLegislatura int, @dataChiusura datetime
	AS
	BEGIN
		BEGIN TRY 
		BEGIN TRANSACTION
	...
	EXEC('CREATE PROCEDURE [dbo].[spAggiornaDataFinePersona] @idPersona int, @idLegislatura int, @dataChiusura datetime,  @isChiusuraLegislatura bit
	AS
	BEGIN
	...
	EXEC('CREATE PROCEDURE [dbo].[spAggiornaDataFineLegislatura] @idLegislatura int, @dataChiusura datetime
	AS
	BEGIN
	...
	EXEC('ALTER PROCEDURE [dbo].[spGetConsiglieri]
		@idLegislatura	int,
		@nome			nvarchar(50),
		@cognome		nvarchar(50)
	AS
	...
	EXEC('ALTER VIEW [dbo].[join_persona_gruppi_politici_incarica_view]
	AS
	...
	COMMIT TRANSACTION
	print('PARTE 2 ESEGUITA CORRETTAMENTE')
	END TRY
	BEGIN CATCH
        ROLLBACK TRANSACTION
		print('ROLLBACK PARTE 2')
	END CATCH
END