...
public partial class report_consiglieri_sostituiti : System.Web.UI.Page
{
	...
	int id_user;
	string title = "Report Consiglieri Sostituiti";
	string tab = "Report: Consiglieri Sostituiti";
	string filename = "Report_Consiglieri_Sostituiti";
	...
	/// <summary>
	/// Query caricamento dati completi consiglieri sospesi
	/// </summary>
	...
	COALESCE(CONVERT(varchar, jps.data_inizio, 103), 'N/A') AS data_inizio,
	...
	COALESCE(CONVERT(varchar, jps.data_fine, 103), 'N/A') AS data_fine,
	...
	INNER JOIN join_persona_sostituzioni AS jpsost
	...
	AND LOWER(jps.tipo) = 'Sostituito da'
	...
	protected void EseguiRicerca()
	{
		...
		if (txt_data_inizio.Text != "")
		{
			...
			query += " AND jps.data_inizio >= '" + dt_inizio_str + "'";
		}
		...
		if (txt_data_fine.Text != "")
		{
			...
			query += " AND jps.data_inizio <= '" + dt_fine_str + "'";
		}
	...
	{
		GridView1.Columns[4].Visible = chkVis_Delibera.Checked;
		GridView1.Columns[5].Visible = chkVis_Causa.Checked;
		GridView1.Columns[6].Visible = chkVis_Note.Checked;
