...
/// <summary>
/// Classe per la gestione Report dati anagrafici Consiglieri
/// </summary>

public partial class report_consiglieri_dati_anagrafici : System.Web.UI.Page
{
    string legislatura_corrente;

    int id_user;
    string title = "Report Consiglieri (Dati Anagrafici)";
    string tab = "Report: Consiglieri - Dati Anagrafici";
    string filename = "Report_Consiglieri_Dati_Anagrafici";
    ...
    string query1 = @"SELECT DISTINCT pp.id_persona
                                     ...
                                     ,COALESCE(jjgv.nome_gruppo, 'NESSUN GRUPPO ASSOCIATO') as nomegruppo
                                     ,dbo.get_gruppi_politici_from_persona (jpoc.id_persona, ll.id_legislatura) as gruppi_tutti
                      FROM persona AS pp
                      ...
                      WHERE pp.deleted = 0
                        ...
                        AND (cc.id_tipo_carica = 4 -- 'consigliere regionale'
                             OR
                             cc.id_tipo_carica = 5 -- 'consigliere regionale supplente'
                            )
    ...
    {
        // opzione di visualizzazione per tutti gruppi
        if (chkVis_Gruppi.Checked)
    ...
    protected void EseguiRicerca()
    {
        ...
        switch (ddlProvinciaResidenza.SelectedValue)
        {
            case "0":
                break;

            default:
                other_condition += " AND tc2.provincia = '" + ddlProvinciaResidenza.SelectedValue + "'";
                //other_condition_query2 += other_condition; 
                break;
        }
        ...
        switch (ddlGruppo.SelectedValue)
        {
            ...
            default:
                other_condition += " AND jjgv.id_gruppo = '" + ddlGruppo.SelectedValue + "'";
                break;
        }
        ...
        //query2 += other_condition_query2;
        //query += other_condition + query2 + query_order;
    ...
    protected void GridView1_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (rowtype == "DataRow")
        {
            string query_recapiti = @"SELECT COALESCE(trec.nome_recapito, 'N/A') AS nome_recapito,
                                             COALESCE(jprec.recapito, 'N/A') AS recapito
    ...
    protected void SetExportFilters()
    {
        ...
        filters[4] = "Provincia Residenza";
        filters[5] = ddlProvinciaResidenza.SelectedItem.Text;
        filters[6] = "Gruppo Consiliare";
        filters[7] = ddlGruppo.SelectedItem.Text;
        filters[8] = "Sesso";
    ...
    protected void genOrderValue()
    {
        ...
        else
        {
            ...
            if (chbOrderDataNascita.Checked)
                if (template.Length > 0)
                    template += ",data";
                else
                    template += "data";

            if (chbOrderComuneNascita.Checked)
                if (template.Length > 0)
                    template += ",tc.comune";
                else
                    template += "tc.comune";

            if (chbOrderComuneResidenza.Checked)
                if (template.Length > 0)
                    template += ",ind2";
                else
                    template += "ind2";
